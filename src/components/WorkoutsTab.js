...
export default function WorkoutsTab({ data }) {
  ...
  return (
    <>
      ...
      {/* Calories per ride */}
      <div className="chart-card">
        <div className="chart-title">{isMobile ? 'Calories Burned — Last 30 Rides' : 'Calories Burned — Last 60 Rides'}</div>
        <ResponsiveContainer width="100%" height={160}>
          <BarChart data={caloriesChart} barSize={5}>
            <XAxis dataKey="date" {...xAxisProps} />
            <YAxis {...yAxisProps} />
